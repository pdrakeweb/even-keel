...
- ✅ **6 HA-bridged scenarios** spanning the full Phase 6 critical-path stack — bilge wet → MQTT discovery → HA `binary_sensor.boat_bilge_water_detected` flips → production `boat_health.yaml` template-sensor rolls up to `sensor.boat_bilge_status="critical"` → production `boat_alerts.yaml` automation creates `persistent_notification.bilge_water`. Clearing reverses every layer. Production HA Docker image, production `home-assistant/` config tree, MQTT integration pre-baked into `.storage` so HA subscribes on boot, onboarding bootstrap via `/api/onboarding/users` → `/auth/token`. The verification step asserts the scenarios actually ran (not silently skipped).
...
**Test surface gating master:** 115 tests across 5 layers — 23 BDD telemetry + 7 simulator integration + 6 HA-bridged + 6 Playwright e2e + 73 custom-card vitest. Plus 164 simulator pytest cases on `simulator-tests.yml`. **279 tests** total when the simulator workflow runs.

**Next:**
- More HA alert coverage — anchor drag, low-SoC battery, shore-power loss, engine over-temp. The `boat_alerts.yaml` skeleton is in place; each new alert is one automation + one BDD scenario.
- Live-mode HMAC handshake + on-boat test_mode injection — turns `--mode=live` from a skeleton into a working stimulus path against deployed firmware (Phase 6 commissioning).
